...
if __name__ == '__main__':
    try:
        ...
        exit(1)
    pat_str = b'%s\t%s' % ('ciscodnacnetbox'.encode(), new_name.encode())
    ...
    pat_str = b'%s\t%s' % ('ciscodnacnetbox'.encode(), new_name.replace('_', '').capitalize().encode())
    patterns = parse_patterns(pat_str)
    rewrite_files(
        root_paths, patterns,
        do_renames=True,
